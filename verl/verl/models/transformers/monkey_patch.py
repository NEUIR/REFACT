...
from verl.models.transformers.nsa import _nsa_forward
...
def _ulysses_nsa_forward(
    ...
    **kwargs,
    ...
    """Insert all-to-all before and after nsa.
    DeepSpeed-Ulysses: https://arxiv.org/pdf/2309.14509
    ...
    # print(f"ulyssess nsa forward position_ids: {position_ids}")
    target_dtype = None
    ...
    if ulysses_sp_size > 1:
        ...
        torch.distributed.all_gather(position_ids_list, position_ids, group=get_ulysses_sequence_parallel_group())
        position_ids = torch.concat(position_ids_list, dim=-1)
    # print(f"after ulysses, position_ids: {position_ids}")

    # (bsz, seq_len, n_head/n, head_dim)
    # print(f"before nsa forward: q, {query_states.shape}, k, {key_states.shape}, v, {value_states.shape}, position_ids: {position_ids}")
    attn_output = _nsa_forward(
        module,
        query_states,
        key_states,
        value_states,
...
def apply_monkey_patch(
    ...
    # transformers<=4.47.1
    if use_remove_padding or ulysses_sp_size > 1:
        if model.config._attn_implementation == "nsa":
            module._nsa_forward = _ulysses_nsa_forward
            print(f"Monkey patch nsa_forward in {model.__module__}")
        elif hasattr(module, "_flash_attention_forward"):
