...
def forward_kernel(
    ...
    hi = min(
        k_len,
        (q_start_in_seq + BLOCK_SIZE_Q - kernel_size) // kernel_stride + 1,
    )
    for i in range(lo, hi, BLOCK_SIZE_K):
        ...
        qk += tl.where(
            off_q[:, None] >= (i * kernel_stride + off_k)[None, :],
            0,
            float("-inf"),
        )
    ...
    # save lse
    l_ptrs = (
        lse_ptr + q_start * stride_ln + pid_h * stride_lh + off_q * stride_ln
    )
...
def backward_sum_o_do(
    ...
    tl.store(
        delta_ptr + pid_h * stride_dh + off_n * stride_dn,
        delta,
        mask=off_n < o_len,
    )
...
def backward_dkdv(
    ...
    for i in range(q_lo, q_len, BLOCK_SIZE_Q):
        ...
        qk = tl.where(
            off_k[:, None] <= (off_q + i)[None, :], float(0.0), float("-inf")
        )
...
def _compressed_attention_fwd(
    ...
    num_warps, num_stages = get_num_warps_stages(
        head_dim, BLOCK_SIZE_Q, IS_HOPPER_GPU
    )
    forward_kernel[grid](
        q,
        k,
...
def _compressed_attention_bwd(
    ...
    # compute D
    delta = torch.zeros(
        [num_o_heads, o_len], device=o.device, dtype=torch.float32
    )
    grid = lambda META: (triton.cdiv(o_len, META["BLOCK_SIZE_O"]), num_o_heads)
    BLOCK_SIZE_O = 256
    BLOCK_SIZE_D = triton.next_power_of_2(head_dim)
    num_warps, num_stages = get_num_warps_stages(
        head_dim, BLOCK_SIZE_O, IS_HOPPER_GPU
    )
    ...
    BLOCK_SIZE_D = triton.next_power_of_2(head_dim)
    num_warps, num_stages = get_num_warps_stages(
        head_dim, BLOCK_SIZE_K, IS_HOPPER_GPU
    )
    backward_dkdv[grid](
        q,
        k,
...
def score_kernel(
    ...
    causal_mask = (
        off_q[:, None] >= (off_k * kernel_stride + kernel_size - 1)[None, :]
    )
    # init score
...
def _transform_score_kernel(
    ...
    tl.store(
        bs_ptrs,
        s,
        mask=(off_q < q_len)[:, None]
        & (off_bk < max_blocks - k_start)[None, :],
    )
...
def transform_score(
    ...
    offs = (
        ...
        + torch.arange(block_size // kernel_stride, device=score.device)[
            None, :
        ]
    ).view(-1)
