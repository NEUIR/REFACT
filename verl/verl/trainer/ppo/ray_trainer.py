...
save_batch_pool = ProcessPoolExecutor(max_workers=8)


def save_batch_async(save_batch, save_file):
    print(f"save_batch async: {save_file}")
    save_batch.save_to_disk(save_file)
    print(f"save_batch done: {save_file}")
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, **kwargs):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        print("compute critic advantages")
...
class RayPPOTrainer:
    ...
    def _dump_generations(self, inputs, outputs, scores, reward_extra_infos_dict, dump_path):
        ...
        # Ensure all data are JSON serializable (convert numpy/torch types)
        import numpy as np  # local import to avoid module-wide dependency if unused elsewhere
        try:
            import torch  # torch may not always be available in minimal environments
        ...
        with open(filename, "w") as f:
            for i in range(n):
                entry = {k: to_jsonable(v[i]) for k, v in base_data.items()}
            ...
            for batch_index, batch_dict in enumerate(self.train_dataloader):
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("reward", timing_raw):
                        ...
                        if self.config.reward_model.launch_reward_fn_async:
                        ...
                        elif self.config.reward_model.get('parallel_reward', False):
                            # pad the batch
                            ...
                            reward_result = self.actor_rollout_wg.verify(batch)
                            # restore the batch
                            restore_idxs = [0] * len(idxs)
                            for i, idx in enumerate(idxs):
                                restore_idxs[idx] = i
                            batch = batch.select_idxs(restore_idxs)
                            ...
                            print(f"[Parallel Reward] reward_extra_infos_dict keys: {list(reward_extra_infos_dict.keys())}")
                            if 'loop_detected' in reward_extra_infos_dict:
                                loop_info = reward_extra_infos_dict['loop_detected']
                                print(f"[Parallel Reward] loop_detected type: {type(loop_info)}, shape/len: {loop_info.shape if hasattr(loop_info, 'shape') else len(loop_info)}")
                                if isinstance(loop_info, np.ndarray):
                                    print(f"[Parallel Reward] loop_detected sum: {loop_info.sum()}, dtype: {loop_info.dtype}")
                            if self.config.data.get('ttrl', False): # 默认parallel 开启训练
                                self.reward_fn.find_ttrl(batch, reward_tensor)
                                new_batch = self.reward_fn.apply_ttrl(batch)
                        else:
                    ...
                    if self.config.data.get("save_batch", False):
                        with _timer("save_batch", timing_raw):
                            # 异步保存batch到文件
                            save_batch = deepcopy(batch)
                            save_file = os.path.join(self.config.trainer.default_local_dir, f"batch_{batch_index}.pickle")
                            save_batch_pool.submit(save_batch_async, save_batch, save_file)
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        # Calculate loop rate for logging (after reward computation)
                        loop_rate = 0.0  # Default value
                        loop_count = 0
                        total_samples = 0
                        if "loop_detected" in batch.non_tensor_batch:
                            loop_detected = batch.non_tensor_batch["loop_detected"]
                            ...
                            print("[Loop Detection] Warning: 'loop_detected' field not found in batch.non_tensor_batch")
                        
                        # Always log loop rate (even if 0)
                        metrics["training/loop_rate"] = loop_rate
                        
                        # Apply loop penalty based on strategy
                        if "loop_detected" in batch.non_tensor_batch:
                            loop_penalty_strategy = self.config.reward_model.get("loop_penalty_strategy", "mask")
                            loop_penalty_value = float(self.config.reward_model.get("loop_penalty_value", 0.0))
                            loop_detected = batch.non_tensor_batch["loop_detected"]
                            response_masks = batch.batch["response_mask"]
                            ...
                            for i in range(len(loop_detected)):
                                ...
                                if is_loop:
                                    if loop_penalty_strategy == "mask":
                                        ...
                                        # This makes GRPO treat it as a failed sample
                                        batch.batch["token_level_scores"][i] = 0.0
                                    elif loop_penalty_strategy == "nothing":
                                        # Strategy 3: Do nothing (just observe loop rate)
                                        pass
                                    elif loop_penalty_strategy == "constant_penalty":
                                        # Strategy 4: Apply constant penalty
                                        current_score = batch.batch["token_level_scores"][i].sum()
                                        new_score = torch.clamp(current_score - loop_penalty_value, min=0.0)
                                        if torch.abs(current_score) > 1e-6:
                                            batch.batch["token_level_scores"][i] *= (new_score / current_score)
                                        else:
                                            batch.batch["token_level_scores"][i] = 0.0
