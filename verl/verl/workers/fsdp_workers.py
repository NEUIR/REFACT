...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str, reward_config=None):
        super().__init__()
        self.config = config
        import torch.distributed
    ...
    def scale_lr_cond(self, name):
        if name.endswith(".weight") and "layer_norm" not in name and "layernorm" not in name and "embedding" not in name \
            and "output_layer" not in name and "lm_head" not in name and "embed_tokens" not in name \
            and "rotary_emb" not in name and ".norm" not in name:
            return True
        else:
            return False

    def create_mup_optimizer(self, actor_module_fsdp, config) -> "torch.optim.Optimizer":
        print("creating mup optimizer")
        from torch.distributed.fsdp import FullyShardedDataParallel as FSDP
        if isinstance(actor_module_fsdp, FSDP):
            model_config = actor_module_fsdp.module.config
        else:
            model_config = actor_module_fsdp.config
        hidden_size = getattr(model_config, config.hidden_size_key)
        mup_base_size = getattr(model_config, config.mup_base_size_key)
        lr_scale = 1 / (hidden_size / mup_base_size)
        param_groups = [
            {'params': [], 'lr': config.lr, 'weight_decay': config.get("weight_decay", 1e-2)},
            {'params': [], 'lr': config.lr, 'weight_decay': 0},
            {'params': [], 'lr': config.lr * lr_scale, 'weight_decay': config.get("weight_decay", 1e-2)},
            {'params': [], 'lr': config.lr * lr_scale, 'weight_decay': 0},
        ]

        for name, param in actor_module_fsdp.named_parameters():
            if not param.requires_grad:
                continue
            if self.scale_lr_cond(name):
                if "bias" in name:
        ...
        print(f"mup param groups: {param_groups}")
        adamw_args = {
            'lr': config.lr,
        ...
        return torch.optim.AdamW(param_groups, **adamw_args)
    ...
    def _build_model_optimizer(
        ...
        if self.config.model.get("use_nsa", False):
            print("initializing nsa model")
            actor_model_config = AutoConfig.from_pretrained(local_path, trust_remote_code=trust_remote_code, attn_implementation="nsa")
        else:
            print("initializing flash attention 2 model")
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if type(actor_model_config) in AutoModelForVision2Seq._model_mapping.keys():
                ...
                # actor_module_class = MiniCPMForCausalLM
        ...
        if role == "actor" and optim_config is not None:
            ...
            if optim_config.get("use_mup", False):
                actor_optimizer = self.create_mup_optimizer(actor_module_fsdp, optim_config)
            else:
    ...
    def verify(self, data: DataProto):
        assert self.reward_config is not None, "reward_config is not provided"
        reward_fn = load_reward_manager(
            self.reward_config,
            self.tokenizer,
            num_examine=0,
            **self.reward_config.reward_model.get("reward_kwargs", {})
        )
        result = reward_fn(data, return_dict=True)
        dic = {k: np.array(v) for k, v in result['reward_extra_info'].items()}
        return DataProto.from_dict(
            tensors={'reward_tensor': result['reward_tensor']},
            non_tensors=dic,
        )

    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def is_garbled(self, data: DataProto):
        from verl.utils.reward_score.garbled import batch_is_garbled
        data.batch["is_garbled"] = torch.tensor(batch_is_garbled(self.tokenizer, data.batch["responses"]), dtype=torch.float).unsqueeze(-1)
        return data
